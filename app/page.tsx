...
import FamilyAlbumView from '@/components/FamilyAlbumView'; // 追加
...
export default function HomePage() {
  ...
  const [view, setView] = useState<'home' | 'album' | 'analytics'>('home'); // 画面管理State
  ...
  return (
    <div className={`flex h-screen w-full bg-[#F8F9FA] overflow-hidden relative transition-all duration-700 ${
      ...
      <div className="absolute top-0 left-0 w-full h-16 flex items-center justify-center pointer-events-none z-10">
        <h1 className="text-2xl font-bold text-gray-700 tracking-wider uppercase">
          {view === 'home' ? 'VocaBoard' : view === 'album' ? 'Family Album' : 'Analytics'}
        </h1>
      </div>
      ...
      <main className="flex flex-1 h-full p-4 pt-16 gap-6 overflow-hidden">
        ...
        {view === 'home' ? (
              ...
              <div className="flex justify-between items-center px-8 pt-6 pb-2 shrink-0">
                ...
                <button
                  ...
                  className={`px-6 py-2 rounded-full text-sm font-bold transition-all active:scale-95 shadow-sm ${
                    isEditMode ? 'bg-red-500 text-white shadow-red-200' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}
                >
                  {isEditMode ? '編集を終了' : '編集'}
                </button>
              </div>
            ...
            <section className="h-full flex flex-col pb-2 min-h-0 w-[500px] flex-shrink-0 gap-3">
              ...
              <div className="flex-1 flex flex-col justify-center px-4">
                ...
                <div className="mt-6 flex flex-col items-center gap-2">
                  {!latestAudioBlob ? (
                    <span className="text-red-400 font-bold flex items-center gap-2 bg-red-50 px-4 py-1 rounded-full text-sm"><Mic size={16} /> 録音データなし</span>
                  ) : notes.length === 0 ? (
                    <span className="text-orange-400 font-bold flex items-center gap-2 bg-orange-50 px-4 py-1 rounded-full text-sm"><PlusCircle size={16} /> メモを描いて追加</span>
                  ) : (
                    <span className="text-emerald-500 font-bold flex items-center gap-2 bg-emerald-50 px-4 py-1 rounded-full text-sm animate-bounce"><CheckCircle2 size={16} /> 解析OK！</span>
                  )}
                </div>
              </div>
            </section>
          ...
          /* アルバム画面：画面全体を贅沢に使う */
          <section className="flex-1 h-full bg-white/60 backdrop-blur-md rounded-[40px] border border-gray-200 shadow-inner overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
            <FamilyAlbumView />
          </section>
        ) : (
          /* 解析グラフ画面 */
          <section className="flex-1 h-full bg-white flex items-center justify-center rounded-[40px] border border-gray-200">
             <div className="text-center">
               <h2 className="text-2xl font-black text-gray-300">Analytics View</h2>
               <p className="text-gray-400">現在、グラフ機能を開発中です...</p>
             </div>
          </section>
        )}
      </main>
